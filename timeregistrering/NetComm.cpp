...
void NetComm::SendButtonEventRequest(int id, unsigned long long unixTimestamp) {
  Serial.println("Send POST");
  if (WiFi.status() != WL_CONNECTED) {
    Serial.println("Cannot send POST request. Connection not active.");
     _mainLed.YellowBlink();
  }
  ...
  if (client.connect("192.168.1.36", 5001)) {
    ...
    if (responseCode != 200) {
      ...
      Serial.println("Event recevied by server");
    }
  }
}
